...
My OSX / Ubuntu dotfiles / GitForWindows.

Dragons lurk within for sure venture here with caution
...
* [Fonts](/sams/dotfiles/tree/master/conf/osx/fonts) via the [init/50_osx_fonts.sh](init/50_osx_fonts.sh) script
...
If you modify things and notice a bug or an improvement, [file an issue](https://github.com/sams/dotfiles/issues) or [a pull request](https://github.com/sams/dotfiles/pulls) and let me know.
...
You might want to set up your ubuntu server [like I do it](https://github.com/sams/dotfiles/wiki/ubuntu-setup), but then again, you might not.
...
1. Fork this repo
1. Open a terminal/shell and do this (change `sams` and `master` as appropriate):

#### Ubuntu

```sh
export DOTFILES_GH_USER=sams
...
```sh
bash -c "$(wget -qO- http://bit.ly/2BnoI6g)" && source ~/.bashrc
```

#### macOS

```sh
bash -c "$(curl -fsSL http://bit.ly/2BnoI6g)" && source ~/.bashrc
```

#### GitForWindows ???

```sh
bash -c "$(curl -??? http://bit.ly/2BnoI6g)" && source ~/.bashrc
```
